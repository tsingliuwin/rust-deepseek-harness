        ...
        for inline_node in children {
            ...
            if let Some(image) = &inline_node.image {
                ...
                for (range, style) in &inline_node.marks {
                    ...
                    if style.bold {
                        // [dsh] web .markdown strong { font-weight: 600 }：
                        // Segoe UI 有真实 Semibold 面，700 明显更黑。
                                ...
                                items.push(
                                    h_flex()
                                        ...
                                        .when(!options.todo && checked.is_none(), |this| {
                                            // [dsh] 对齐 web .markdown li::marker
                                            // { color: label-secondary }：标记用
                                            // 次级色，正文保持主色。
                                            this.child(
                                                div()
    ...
    fn render_block(
        ...
        match self {
            ...
            Node::Paragraph(paragraph) => div()
                ...
                .into_any_element(),
            Node::Heading { level, children } => {
                // [dsh] 标尺对齐 web --dsw-font-markdown-h*：h1-h3 700、
                // h4-h6 600（上游默认 h2/h3 只有 600、h6 500）。
                ...
                // [dsh] 行高随标尺（web h1-h3 30/28/26px，h4-h6 继承 24px）。
                let mut line_height = None;
                if let Some(f) = node_cx.style.heading_line_height.as_ref() {
                    line_height = Some((f)(*level, node_cx.style.heading_base_font_size));
                }

                // [dsh] 标题边距对齐 web .markdown h1-h3 { margin: 32px 0 16px }
                // / h4-h6 { margin: 16px 0 }：上游固定 pb(0.3rem) 让标题几乎
                // 贴住后续内容（上方又只有前一块的 16px，h1-h3 应为 32px）。
                let mut heading = h_flex()
                    .id(("h", *level as usize))
                    .when(*level <= 3, |this| this.pt(px(16.)))
                    .pb(px(16.))
                    .whitespace_normal()
                    .text_size(text_size)
                    .font_weight(font_weight);
                if let Some(lh) = line_height {
                    heading = heading.line_height(lh);
                }
                heading
            ...
            Node::Blockquote { children } => div()
                ...
                .child(
                    div()
                        .id("blockquote")
                        .w_full()
                        // [dsh] 对齐 web .markdown blockquote：2px caption 左
                        // 边线 + padding-left 14px，正文不再压暗（web 文本
                        // 保持 label-primary）。
                        .border_l_2()
            ...
            Node::List { children, ordered } => v_flex()
                ...
                // [dsh] 对齐 web .markdown li:not(:first-child) { margin-top: 6px }
                // （嵌套列表 web 为 4px，此处统一 6px）。
                .gap(px(6.))
                .pb(mb)
                .children({
